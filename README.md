...
Agent-ergonomics behavior: `idempotencyKey` may be omitted on `cycles_commit` and `cycles_release` only — the server generates one (`mcp_<uuid>`), and fresh-key retries are protected by the reservation state machine (409 `RESERVATION_FINALIZED`, never a duplicate effect). All other mutating tools keep caller-supplied keys **required**: for `cycles_reserve`/`cycles_create_event` the key is the only retry deduplication (duplicate hold / double charge otherwise); a fresh-key `cycles_extend` retry double-extends; and every `cycles_decide` emits a signed evidence artifact, so only a same-key replay avoids duplicate audit entries. Explicit subject fields always win over `CYCLES_DEFAULT_*` values. Responses carry plain-text hints after the JSON payload when the budget is under pressure (DENY, `ALLOW_WITH_CAPS`, or under ~15% remaining), so agents self-regulate without host support.
