...
**Review findings (external, 2026-07-22) — all three confirmed and fixed:** (1) **P1, the important one:** auto-generated keys destroy retry deduplication — a retried call gets a fresh key and duplicates its effect, contradicting `idempotentHint: true`. Fixed by a per-tool safety matrix grounded in the spec: keys stay REQUIRED on `cycles_reserve` and `cycles_create_event`, where the key is the ONLY dedup for new budget effects (duplicate hold / double charge); keys remain optional on `cycles_decide` (no persistent effect), `cycles_commit`/`cycles_release` (spec mandates 409 `RESERVATION_FINALIZED` on re-finalization — retries fail loud, never double-apply), and `cycles_extend` (server-capped via `MAX_EXTENSIONS_EXCEEDED`). (2) env defaults bypassed the Zod schema: oversized or whitespace-only values could reach the wire — now validated at merge (non-blank, <=128 chars) with an error naming the offending env var. (3) packaged installs couldn't configure the defaults — the six fields are now exposed in the MCPB manifest `user_config` (+ env mappings), `smithery.yaml`, and `server.json` registry metadata; empty-string values from unset UI fields are ignored by the merge.
