...
**Verified:** 198 tests (key generation format asserted at the adapter boundary on a retry-safe tool, env-default merge + explicit-wins, oversized and whitespace-only defaults rejected with named errors, no-defaults still rejects, empty balance call with defaults, all three hint types, structuredContent purity, no hints on healthy responses); coverage 98.54% lines; MCPB manifest re-validated; typecheck + lint clean.
