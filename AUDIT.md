...
**Review findings (external, 2026-07-21), both confirmed and fixed:** (1) the job's explicit permissions block zeroed `actions`, so the `gh workflow run` dispatch would 403 — `actions: write` added; (2) PRs created by `GITHUB_TOKEN` don't trigger `pull_request` workflows, and main's branch protection requires the ci.yml `test / Test (Node 20/22)` checks — the release PR would be unmergeable. Fixed without a PAT: the workflow also dispatches ci.yml at the release branch, attaching the required check runs to the PR's head SHA (checks are commit-scoped; publish jobs stay skipped off-tag). **Outstanding prerequisite:** "Allow GitHub Actions to create and approve pull requests" must be enabled at the ORG level (repo-level API attempt returned 409: org disallows) and then at the repo level, or release-please cannot open its PR. Until then the workflow is inert but harmless.
