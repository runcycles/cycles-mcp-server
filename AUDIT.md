...
**Design:** release-please (pinned v4.4.1 by SHA) maintains a release PR from conventional commits on `main`. Version sync: `extra-files` JSON updaters cover both `server.json` version fields; a generic updater bumps the AUDIT.md header via an inline release-please version annotation (the literal token appears only on the header line — writing it in prose here would make the updater rewrite this paragraph's version strings too, which is exactly what happened to this paragraph in the first generated release PR). Changelog sections map conventional types onto the existing Added/Fixed/Changed vocabulary. Merging the release PR creates the tag and GitHub release.
...
## Agent Ergonomics — Subject Defaults, Budget Hints (2026-07-22)

**Files:** `src/schemas.ts`, `src/tools/util.ts`, all 7 tool handlers, `tests/`, `README.md`.

**Spec-conformance note (spec remains the authority):** the protocol REQUIRES `idempotency_key` on mutating calls and a subject on reserve/decide/events — and every wire request still carries them. What changed is only the MCP-facing input schema (client-side policy): `subject` is optional on reserve/decide/create_event, merged from operator-configured `CYCLES_DEFAULT_TENANT/WORKSPACE/APP/WORKFLOW/AGENT/TOOLSET` env vars (explicit fields win; dimensions never defaulted; the ≥1-standard-field validation still runs post-merge and its error now points at the env defaults). `cycles_check_balance` accepts an empty call when defaults supply a filter.
...
**Review findings (external, 2026-07-22) — all three confirmed and fixed:** (1) **P1, the important one:** auto-generated keys destroy retry deduplication — a retried call gets a fresh key and duplicates its effect, contradicting `idempotentHint: true`. Fixed by a per-tool safety matrix grounded in the spec: keys stay REQUIRED on `cycles_reserve` and `cycles_create_event`, where the key is the ONLY dedup for new budget effects (duplicate hold / double charge); keys remain optional ONLY on `cycles_commit`/`cycles_release` (spec mandates 409 `RESERVATION_FINALIZED` on re-finalization — retries fail loud, never double-apply, and are recorded as error artifacts rather than duplicate lifecycle claims). A second review round narrowed the matrix further: `cycles_extend` reverted to required (the MAX_EXTENSIONS cap is quota, not dedup — a fresh-key retry double-extends and burns extension allowance) and `cycles_decide` reverted to required (per cycles-evidence-v0.2, decide is one of the four signed evidence-emitting lifecycle events — a fresh-key retry mints a duplicate signed audit artifact; only a same-key replay suppresses emission). A third round removed auto-generation entirely: even on commit/release, a fresh-key retry returns 409 `RESERVATION_FINALIZED` instead of replaying the stored success AND mints an additional signed error artifact — an environmental effect that contradicts `idempotentHint: true`. Final state: **`idempotencyKey` is caller-supplied and required on every mutating tool**; the shipped ergonomics are subject env defaults, the relaxed `cycles_check_balance`, and budget-pressure hints. (2) env defaults bypassed the Zod schema: oversized or whitespace-only values could reach the wire — now validated at merge (non-blank, <=128 chars) with an error naming the offending env var. (3) packaged installs couldn't configure the defaults — the six fields are now exposed in the MCPB manifest `user_config` (+ env mappings), `smithery.yaml`, and `server.json` registry metadata; empty-string values from unset UI fields are ignored by the merge.

**Verified:** 197 tests (env-default merge + explicit-wins asserted at the adapter boundary, oversized and whitespace-only defaults rejected with named errors, no-defaults still rejects with a message pointing at the env vars, empty balance call with defaults, all three hint types, structuredContent purity, no hints on healthy responses, explicit key passthrough); coverage 98.53% lines; MCPB manifest re-validated; typecheck + lint clean.

---

## Dependabot Auto-Merge — Squash Method Fix (2026-08-06)

**File:** `.github/workflows/dependabot-auto-merge.yml`. **One-line behavior fix, no scope/permission changes.**

The automerge job ran `gh pr merge --auto --merge`, but this repo was switched to squash-only merges on 2026-07-21 (see "Third review round" above — merge commits duplicated changelog entries). Since then every Dependabot PR failed the job with `GraphQL: Merge method merge commits are not allowed on this repository (enablePullRequestAutoMerge)` and sat unmerged despite green CI — observed on #178 (fast-uri 3.1.5 security patch, merged manually 2026-08-06). Fixed by changing the flag to `--squash`, matching the repo's only allowed merge method; squash titles come from PR titles (`PR_TITLE` config, per the release-please changelog fix), and Dependabot PR titles are conventional, so changelog parsing is unaffected. Patch-only gating, metadata pinning, and the AUTOMERGE_PAT fallback are unchanged.
