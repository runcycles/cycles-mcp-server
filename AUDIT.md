...
**MCP Server:** `@runcycles/mcp-server` v0.4.0 (Node 20+ / `@modelcontextprotocol/sdk` / TypeScript 6)
...
**Review finding (external, 2026-07-21) — license compliance:** the initial bundle shipped no license texts: Apache-2.0 §4(a) requires recipients receive a copy of the project LICENSE, and the inlined MIT dependencies' permission notices were stripped by the bundler (an earlier self-review pass wrongly graded this compliant based on a weak grep — the bundle contained zero full permission notices). Fixed: the archive now includes `LICENSE` and a generated `THIRD-PARTY-NOTICES.md` concatenating the full license text of every inlined production package from the lockfile (94 packages, 0 without license files, verified at build time). **Still open — `privacy_policies` manifest field:** MCPB v0.3 conditionally requires privacy-policy URLs when the extension connects to external services processing user data, which this does. runcycles.io currently publishes no privacy policy (verified 2026-07-21: `/privacy` is 404, no legal links on the site), so the field is deliberately absent rather than pointing at a fabricated URL. Resolved same day: privacy policy drafted and PR'd to cycles-docs (runcycles/cycles-docs#814, serving /privacy on deploy), and `privacy_policies: ["https://runcycles.io/privacy"]` added to the manifest (validates against the spec). The docs PR must merge and deploy before any extension-directory submission so the URL resolves.
